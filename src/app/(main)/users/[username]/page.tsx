...
import { FollowerInfo, getUserDataSelect, type UserData } from "@/lib/types"
...
export async function generateMetadata({params:{username}}:PageProps):Promise<Metadata>{
  ...
  return {
    ...
    description:`@${user.username} has ${user._count.fallowers} followers`
  }
}
...
const ProfilePage = async({params:{username}}:PageProps) => {
  ...
  if(!loggedInUser) return (
    <div className="text-destructive">Not logged in</div>
  )
...
const UserProfile = async({user,loggedInUserId}:UserProfileProps) => {
  const followerInfo:FollowerInfo = {
    ...
    isFollowedByUser:user.fallowers.some(
